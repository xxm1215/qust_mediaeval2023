...
model.load_state_dict(torch.load("model_vacc_all@rt.pt"))
model.to(device)
dataset = MediaEval24_Test_Dataset("./rt_test_dataset.json")
...
for txt, img, t_idx, i_idx in tqdm(test_loader):
    ...
    for idx in range(len(t_idx)):
        ...
        c += 1
        with open("/mnt/qust_521_big_2/public/MediaEval2023/predictions/RT/{}_{}.json".format(img_name, c + 1),
                  'w') as out_f:
