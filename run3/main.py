...
model.to(device)
dataset = MediaEval24_Dataset("./expanded_dataset_0.0004.json")
y = dataset.labels
print(len(y))
...
for idx, (train_idx, val_idx) in enumerate(skf.split(dataset, y)):

    print("="*10 + "Fold: {}".format(idx + 1) + "="*10)
    print("No. of Train: ", len(train_idx))
    print("No. of Test: ", len(val_idx))
    ...
    best_val_acc = 0
    iteration_number = 1
    train_fold = Subset(dataset=dataset, indices=train_idx)
    val_fold = Subset(dataset=dataset, indices=val_idx)

    train_loader = DataLoader(train_fold, batch_size=28, shuffle=True)
    val_loader = DataLoader(val_fold, batch_size=28, shuffle=True)
    ...
    # split = n//5
    #
    # train_sampler = SubsetRandomSampler(idx[split:])  # 80%
    # test_sampler = SubsetRandomSampler(idx[:split])  # 20%


    # train_loader = DataLoader(dataset, sampler=train_sampler, batch_size=28)
    # test_loader = DataLoader(dataset, sampler=test_sampler, batch_size=28)
    ...
    for epoch in range(EPOCH):
        for txt, img, label in tqdm(train_loader):
            ...
            loss = loss_contrastive + loss_cossim + loss_crossent + loss_focal
            ...
            writer.add_scalar("fold{}_total_loss/train".format(idx+1),  loss.detach().item(), iteration_number)
            writer.add_scalar("fold{}_contrastive_loss/train".format(idx+1),loss_contrastive.detach().item(), iteration_number)
            writer.add_scalar("fold{}_cosin_loss/train".format(idx+1),loss_cossim.detach().item(), iteration_number)
            writer.add_scalar("fold{}_bce_loss/train".format(idx+1), loss_crossent.detach().item(), iteration_number)
