...
def process_text(json_file_path, model_name):
    ...
    if os.path.exists('RT4_text.pickle'):
        with open('RT4_text.pickle', 'rb') as f:
            indexes = pickle.load(f)
    ...
    for item in data:
        ...
        text = item["titleEN"]  # 获取JSON文件中的"text"值
    ...
    for i in tqdm.tqdm(range(0, len(data), BATCH_SIZE)):
        ...
        batch_text = [item["titleEN"] for item in batch_data]
    ...
    # 保存到pickle文件中
    with open('RT4_text.pickle', 'wb') as f:
        pickle.dump(indexes, f)
...
if __name__ == '__main__':
    ...
    json_file_path = '/home/qd/PycharmProjects/MediaEval2022/rt-test.json'  # 替换为您的JSON文件路径
